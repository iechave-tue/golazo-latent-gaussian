...
ggplot(data=df, aes(x=lambda)) +
  geom_line(aes(y=lasso,color="Graphical lasso"),linewidth=0.8)+
  geom_line(aes(y=lassoggm,color="Graphical lasso with constraints"),linewidth=0.8)+
  geom_line(aes(y=positive,color="Positivity"),linewidth=0.8)+
  geom_line(aes(y=positiveggm,color="Positivity with constraints"),linewidth=0.8)+
...
ggplot(data=df, aes(x=lambda)) +
  geom_line(aes(y=lasso,color="Graphical lasso"),linewidth=0.8)+
  geom_line(aes(y=lassopositive,color="Modified positivity"),linewidth=0.8)+
  geom_line(aes(y=positivelasso,color="Positive lasso"),linewidth=0.8)+
  geom_line(aes(y=positive,color="Positivity"),linewidth=0.8)+
  labs(x=TeX("Value of $\\lambda$"),y="Log-likelihood",color="Legend")+
  theme(legend.position = c(.7,.50))+
  scale_color_manual(labels = c(TeX("Standard $\\ell_1$-penalty"),
                                TeX("Modified $\\ell_1$-penalty"),
                                TeX("Positive lasso"),
                               TeX("${MTP}_2$ constraint")),
